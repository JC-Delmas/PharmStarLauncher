...
# If user chooses to lift over the VCF files
        if [ $answer -eq 0 ]; then
                ...
                do
                        ...
                        then
                                # After the liftover, check for lines containing "alt"
                                if grep -q 'alt' "$output_lifted_file"; then
                                        dialog --title "Détection de contigs alternatifs" --yesno "Des contigs alternatifs ont été identifiés dans le fichier VCF traité. \nLeur présence peut engendrer des erreurs ultérieures, il est recommandé de les supprimer.\n\nVoulez-vous les éliminer maintenant ?" 10 60
                                        if [ $? -eq 0 ]; then
                                                grep -v 'alt' "$output_lifted_file" > "${output_lifted_file}.tmp" && mv "${output_lifted_file}.tmp" "$output_lifted_file"
                                                dialog --title "Suppression effectuée" --msgbox "Les contigs alternatifs ont été éliminés du fichier." 5 60
                                        else
                                                dialog --title "Attention" --msgbox "Les contigs alternatifs seront conservés.\nCela peut compromettre la fiabilité des résultats.\nConsidérez l'utilisation de la version HG38 pour une meilleure compatibilité." 10 60
                                        fi
                                fi
                        else
